...
objectdef brrUplink
{
    ...
    method Initialize()
    {
        LGUI2:LoadPackageFile[xBRR.Uplink.lgui2Package.json]
        Settings:Store
...
;            echo "[BRR] Enabling"
    ...
    method GenerateItemView_Profile()
	{
       ; echo GenerateItemView_Game ${Context(type)} ${Context.Args}
		...
		joListBoxItem:SetValue["${LGUI2.Template["BRRUplink.profileView"].AsJSON~}"]
...
;       echo GenerateItemView_Override[${BRRUplink.EditingProfile.Name~}] ${Context(type)} ${Context.Args}
		...
		; build an itemview lgui2element json
		variable jsonvalue joListBoxItem
		joListBoxItem:SetValue["${LGUI2.Template["BRRUplink.overrideView"].AsJSON~}"]
